...
$getLastAccessSQL = $con->prepare("SELECT ult_ini FROM jugador WHERE id_jugador = ?");
$getLastAccessSQL->execute([$id_jugador]);
$last_access = $getLastAccessSQL->fetchColumn();

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $rol_nombre = $_POST['rol'];

    // Insertar nuevo rol
    $insertRolSQL = $con->prepare("INSERT INTO rol (rol) VALUES (?)");
    if ($insertRolSQL->execute([$rol_nombre])) {
        echo '<script>alert("Nuevo rol agregado exitosamente");</script>';
    } else {
        echo '<script>alert("Error al agregar nuevo rol");</script>';
    }
}
?>
