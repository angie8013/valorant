...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['nombre']) && isset($_FILES['mundo'])) {
        // Recuperar los datos del formulario
        $nombre = $_POST['nombre'];
        $mundo = $_FILES['mundo'];

        // Verificar si ya existe un mundo con el mismo nombre
        $stmtCheckName = $con->prepare("SELECT * FROM mundo WHERE nombre = ?");
        $stmtCheckName->execute([$nombre]);
        if ($stmtCheckName->rowCount() > 0) {
            // Ya existe un mundo con este nombre
            $error_message = "Ya existe un mundo con este nombre.";
        } else {
            // Obtener el contenido binario de la imagen
            $mundoBinario = file_get_contents($mundo['tmp_name']);

            // Preparar la consulta SQL para insertar los datos en la tabla mundo
            $sql = "INSERT INTO mundo (nombre, mundo) VALUES (:nombre, :mundo)";
            $stmt = $con->prepare($sql);
            $stmt->bindParam(':nombre', $nombre);
            $stmt->bindParam(':mundo', $mundoBinario, PDO::PARAM_LOB);

            // Ejecutar la consulta
            if ($stmt->execute()) {
                // Mensaje de confirmación
                $success_message = "El mundo se agregó correctamente.";
            } else {
                // Error en la ejecución de la consulta
                $error_message = "Error al ingresar el mundo.";
            }
        }
    } else {
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                    <h2   style="margin-bottom: 20px; color: #333;"><strong>Formulario Mundos</strong></h2>
                    </div>
                </div>
                <hr />
                <!-- /. ROW  -->
                            ...
                            <h3>Registrar Mundo</h3>
                                <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="file" class="form-control-file" id="mundo" name="mundo" accept="image/*" required>
                                    </div>

                                    <button type="submit" class="btn btn-primary">Guardar Mundo</button>
                                </form>
                            </div>
                        </div>
