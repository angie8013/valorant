...
// Consulta SQL para obtener los datos de la tabla "mundo"
$sql = "SELECT * FROM mundo";
$result = $con->query($sql);

// Verificar si se produjo un error en la consulta
if (!$result) {
    // Manejar el error, por ejemplo, mostrando un mensaje de error y saliendo del script
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                    <h2   style="margin-bottom: 20px; color: #333;"><strong>Tabla de Mundos</strong></h2>
                    </div>
                </div>
                <hr />
                <!-- /. ROW  -->
                                        ...
                                        <tbody>
                                            ...
                                            if ($result->rowCount() > 0) {
                                                while ($fila = $result->fetch(PDO::FETCH_ASSOC)) {
                                                    echo "<tr>";
                                                    echo "<td>" . $fila['id_mundo'] . "</td>";
                                                    echo "<td>" . $fila['nombre'] . "</td>";
                                                    echo "<td><img src='data:image/jpeg;base64," . base64_encode($fila['mundo']) . "' width='180' height='130'></td>";
                                                    echo "<td><a href='../update/editar_mundos.php?id=" . $fila['id_mundo'] . "' class='btn btn-primary'  onclick=\"window.open('../update/editar_mundos.php?id={$fila['id_mundo']}','','width=500,height=500,toolbar=NO'); return false;\">Editar</a></td>"; // Botón de editar con enlace a la página de edición
                                                    echo "</tr>";
                                                }
                                            } else {
