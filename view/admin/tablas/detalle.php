...
// Consulta SQL para obtener los detalles de la batalla
$sql = "SELECT detalle.*, jugador_atacante.nombre AS nombre_atacante, jugador_atacado.nombre AS nombre_atacado, arma.nombre AS nombre_arma, agente.nombre AS nombre_agente
        FROM detalle_batalla AS detalle
        INNER JOIN jugador AS jugador_atacante ON detalle.id_jugador_atacante = jugador_atacante.id_jugador
        INNER JOIN jugador AS jugador_atacado ON detalle.id_jugador_atacado = jugador_atacado.id_jugador
        INNER JOIN arma ON detalle.id_arma = arma.id_arma
        INNER JOIN agente ON detalle.id_agente = agente.id_agente";
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                    <h2   style="margin-bottom: 20px; color: #333;"><strong>Tabla de Detalles</strong></h2>
                    </div>
                </div>
                <hr />
                <!-- /. ROW  -->
                                        ...
                                        <tbody>
                                            ...
                                            if ($resultado->rowCount() > 0) {
                                                ...
                                                while ($fila = $resultado->fetch(PDO::FETCH_ASSOC)) {
                                                    ...
                                                    echo "<td>" . $fila['nombre_atacante'] . "</td>";
                                                    echo "<td>" . $fila['nombre_atacado'] . "</td>";
                                                    echo "<td>" . $fila['nombre_arma'] . "</td>";
                                                    echo "<td>" . $fila['nombre_agente'] . "</td>";
                                                    echo "<td>" . $fila['puntos_vida'] . "</td>";
                                                    echo "<td><a href='../update/editar_detalle.php?id=" . $fila['id_detalle'] . "' class='btn btn-primary'  onclick=\"window.open('../update/editar_detalle.php?id={$fila['id_detalle']}','','width=500,height=500,toolbar=NO'); return false;\">Editar</a></td>"; // Botón de editar con enlace a la página de edición
                                                    echo "</tr>";
                                                }
                                            } else {
                                                echo "<tr><td colspan='6'>No se encontraron detalles de batalla</td></tr>";
                                            }
                                            ?>
                                        </tbody>
                                    </table>
